...
void *malloc(size_t size) 
{
   ...
   {
      //define a struct block make the 
      ...
      next->size = size;
      next->next=TNBlock;
      //increment num_splits & the num_blocks
      num_splits++;
      num_blocks++;
   }
   //make the next block BE NOT free
   next->free=false;
   //increment num_mallocs
   num_mallocs++;
   //then increment the num_mallocs 
 ...
 * \return none
*/
void free(void *ptr) //already have a pointer
{
